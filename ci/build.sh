...
tar -xzf sdist-test/just-json-server-*.tar.gz -C sdist-test --strip-components=1

(echo "::endgroup::"; echo "::group::Build and test just-json-server") 2>/dev/null

pushd sdist-test
$STACK build $STACK_OPTS

# Strip the binary, and copy it to the staging directory
echo "gather the binary file"
FULL_BIN="$(stack exec which just-json-server)"
if [ "$OS" != "win64" ] 
then
  strip "$FULL_BIN"
fi
...
#   # ls -l "$FULL_BIN"
#   # 7z --help
#   # 7z a "built-bin/$OS.zip" "$FULL_BIN"
#   cp "$FULL_BIN" "built-bin"
# else
#   zip -rj "built-bin/$OS.zip" "$FULL_BIN"
# fi
mkdir -p "built-bin"
cp -r "$FULL_BIN" "built-bin/$OS-$(basename $FULL_BIN)"
